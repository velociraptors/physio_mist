...
void Physio_MIST::Variables_Form::attach_structure(){
	...
	if(path != "" && path != "ontology"){
		...
		Var->component->set_anatomical_structure(old_str);
	}
}

/// Accepts names containing A-Z, a-z, 0-9, _ (must start with a letter)
bool Physio_MIST::Variables_Form::validName(System::String ^name, [Out]interior_ptr<String^> errorMessage){
	if(name->Length == 0){
...
/// TODO: \todo figure out formula validation (should anything be done here? or is the validation in Model_Form enough?
bool Physio_MIST::Variables_Form::validFormula(System::String ^formula, [Out]interior_ptr<String^> errorMessage){
	*errorMessage = "";
	return true;
}

/// Accepts integer and decimal values, as well as variations of "infinity"
bool Physio_MIST::Variables_Form::validInitVal(System::String ^value, [Out]interior_ptr<String^> errorMessage){
	if(value->Length == 0) {
		// if blank, default to 0
		initValTextBox->Text = "0";
		*errorMessage = "";
		return true;
	}
...
bool Physio_MIST::Variables_Form::validUnits(System::String ^units, [Out]interior_ptr<String^> errorMessage){
	*errorMessage = "";
	return true;
}
...
bool Physio_MIST::Variables_Form::validDesc(System::String ^desc, [Out]interior_ptr<String^> errorMessage){
	*errorMessage = "";
	return true;
}
